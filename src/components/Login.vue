    ...
    <div id="bg-div">
      ...
      <div class="input-group-un">
        <div class=" maxWidth m-auto to80">
          ...
          <input class="form-control form-control-lg credentials-text " type="text" placeholder="enter username">
        </div>
      </div>
      <div class="input-group-password">
        <div class="input-group-password maxWidth m-auto to80">
          <label class="lblCredentials">Password</label>
          <input type="password" class="form-control form-control-lg credentials-text " placeholder="enter password">
        </div>
      </div>
      <div id="btnLoginDiv" class="m-auto to80">
        <button @click="showError" id="btnLogin" class="btn mt-50 "><i class="material-icons">arrow_forward</i></button>
      </div>
      ...
      <transition name="fade">
        <div id="dimScreen" v-show="hassError">
          <div class="notif-text-container text-center">
            ...
            <button class="btn btnOk" @click="login">OK</button>
          </div>
        </div>
      </transition>
    </div>
...
export default {
  ...
  methods: {
    ...
    login () {
      let data = {
        client_id: 3,
        client_secret: 'K32Gv1lkcvBt8RLw8Mbfsj96UoGdfQHARRqeZQDY',
        grant_type: 'password',
        username: 'tmaggio@example.net',
        password: 'secret'
      }
      let self = this
